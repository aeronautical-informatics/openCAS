...
/// Therefore, this module 'digitalizes' all files and makes them available at runtime without any further file parsing.
#[allow(dead_code)]
...
impl HCas {
    /// HorizontalCAS consists of 40 different neural networks (smaller network = les runtime).
    /// The splitting parameters are:
    ///
    ...
    /// + `theta`[rad]: angle from homeships heading to intruder in the mathematical sense (counterclockwise)
    /// + `psi` [rad]:  angle of intruder relative to flight direction of the homeship (mathematical sense => counterclockwise)  
    ///
    /// Example:
    ///
    /// `theta` = 5° => intruder is slidely on the left of the homeships heading.
    ///`psi` = 90° / pi/2 => intruder is flying to the left perpendicular to the homeships heading. See [figure 3](https://arxiv.org/pdf/1912.07084.pdf).
    pub fn process(
        &mut self,
        tau: Time,
        range: Length,
